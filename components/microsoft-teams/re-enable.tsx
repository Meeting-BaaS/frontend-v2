...
import {
  type ReEnableTeamsWorkspaceRequest,
  type TeamsWorkspace,
  type TeamsWorkspaceSingleResponse,
  teamsWorkspaceSingleResponseSchema
} from "@/lib/schemas/teams-workspaces"

interface ReEnableTeamsWorkspaceDialogProps {
  workspace: TeamsWorkspace
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ReEnableTeamsWorkspaceDialog({
  workspace,
  open,
  onOpenChange
}: ReEnableTeamsWorkspaceDialogProps) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const response = await axiosPatchInstance<
        ReEnableTeamsWorkspaceRequest,
        TeamsWorkspaceSingleResponse
      >(
        UPDATE_TEAMS_WORKSPACE(workspace.workspace_id),
        { state: "active" },
        teamsWorkspaceSingleResponseSchema
      )
      ...
      <DialogContent className="sm:max-w-md" showCloseButton={!loading}>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            underlying issue with the tenant or its accounts.
          </DialogDescription>
        </DialogHeader>
