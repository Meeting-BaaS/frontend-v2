...
interface AdminRemoveMemberDialogProps {
  member: TeamMember
  open: boolean
...
export function AdminRemoveMemberDialog({
  member,
  open,
  onOpenChange,
  onSuccess
}: AdminRemoveMemberDialogProps) {
  ...
  const handleRemove = async () => {
    ...
    try {
      await removeMember.mutate({
        memberId: member.userId?.toString() ?? member.email,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md" showCloseButton={!removeMember.isPending}>
        <DialogHeader>
          <DialogTitle>Remove User</DialogTitle>
          <DialogDescription>
            Are you sure you want to remove <strong>{member.email}</strong>? They will lose access
            to the app.
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        >
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button type="button" variant="outline" disabled={removeMember.isPending}>
                Cancel
              </Button>
            </DialogClose>
            <Button
