    ...
    try {
      await authClient.signIn.social(
        ...
        },
        {
          ...
          onError: (ctx) => {
            if (ctx.error.code === "NOT_INVITED" || ctx.error.message?.includes("NOT_INVITED")) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6 pt-6" noValidate>
        <FormFields loading={loading} formType="sign-up" showEmailPassword={showEmailPassword} />
        <CallbackError error={callbackError as keyof typeof errorDescription} />
        {!hasSignInMethods ? (
          <p className="text-sm text-muted-foreground">
            No sign-up methods configured. Contact your administrator.
          </p>
        ) : (
          <motion.div
