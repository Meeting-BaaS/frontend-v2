...
import type { BotScreenshot, GetBotScreenshotsResponse } from "@/lib/schemas/bots"
import { getBotScreenshotsResponseSchema } from "@/lib/schemas/bots"

interface ScreenshotsProps {
  botUuid: string
  endedAt: string | null
  /** When provided (e.g. admin bot details), use this URL instead of BFF route so team check is skipped */
  getScreenshotsUrl?: (botId: string) => string
}

export function Screenshots({
  botUuid,
  endedAt,
  getScreenshotsUrl = GET_BOT_SCREENSHOTS
}: ScreenshotsProps) {
  ...
  const handleViewScreenshots = async () => {
    ...
    try {
      ...
      const url = getScreenshotsUrl(botUuid)

      do {
        const response: GetBotScreenshotsResponse =
          await axiosGetInstance<GetBotScreenshotsResponse>(url, getBotScreenshotsResponseSchema, {
            params: {
              cursor: cursor ?? null,
              limit: 250
            }
