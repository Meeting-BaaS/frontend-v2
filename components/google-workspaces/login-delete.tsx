...
import { DELETE_MEET_LOGIN } from "@/lib/api-routes"
import { genericError } from "@/lib/errors"
import type { MeetLogin } from "@/lib/schemas/meet-logins"

interface LoginDeleteDialogProps {
  open: boolean
  login: MeetLogin
  onOpenChange: (open: boolean) => void
}

export function LoginDeleteDialog({ open, onOpenChange, login }: LoginDeleteDialogProps) {
      ...
      <DialogContent className="sm:max-w-md" showCloseButton={!loading}>
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Click the button to delete the login.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit} noValidate>
          <div className="space-y-6 pb-8">
            <div className="flex flex-col gap-1">
              <p>
                Are you sure you want to delete <strong>{login.email}</strong>?
              </p>
              <p className="text-sm text-destructive">
                <span className="font-bold">Warning:</span> This action cannot be undone. Bots using
                this login will no longer be able to authenticate.
              </p>
            </div>
            <FieldGroup>
              <Field>
