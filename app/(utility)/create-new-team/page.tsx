...
export default async function CreateNewTeamPage({ searchParams }: CreateNewTeamPageProps) {
  ...
  const plansResponse = await axiosGetInstance<PlansResponse>(GET_PLANS, plansResponseSchema, {
    headers: {
      Cookie: cookieStore.toString()
    }
  })
  ...
  const availablePlans = plansResponse.data.plans.filter((plan) => plan.type !== "payg")

  // Validate search params with Zod schema
  const searchParamsObj = await searchParams
  const { data: validatedSearchParams } = createNewTeamSearchParamsSchema.safeParse(searchParamsObj)
